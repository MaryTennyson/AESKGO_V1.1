...
@HiltViewModel
class LoginPageViewModel @Inject constructor(val firebaseAuthRepo: FirebaseAuthRepository, val sharedPreferencesService: SharedPreferencesService) :
    ViewModel() {
    ...
    fun signInUser(user: LoginUser) {
        _authState.value = AuthResults.Loading
        signInUser(user)
    }

    fun signUpUser(user: LoginUser) {
        _authState.value = AuthResults.Loading
        signUpButton(user)
    }

    fun signInButton(user: LoginUser) {
    ...
    fun signUpButton(user: LoginUser) {
        viewModelScope.launch {
